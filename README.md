...
radio control plane. The shelf carries three kinds of entry: **12 DJ personas** (portable,
byte-valid exports of a GenWave DJ's personality — name, voice, lore, taste rules, pronunciation
corrections), **4 themes** (station-wide look-and-feel manifests), and **1 font pack** (vendored
webfont faces for the Wardrobe) — each byte-valid and ready for a station to drop straight in.
Personas and themes are open to community submission; font packs are Dean-curated only (see
...
tools/
  ...
  lint.py                      # CI + local: persona submission-length budgets + dead pronunciation rules
...
`<slug>.theme.json`, or `<slug>.font.json` (`tools/catalog_lib.py`'s `KIND_SUFFIXES`); persona wins
if, bizarrely, more than one is present, then theme, then font. A font pack's `OFL.txt` is tracked
`-text` in [`.gitattributes`](./.gitattributes) — licence text is byte-hashed into `index.json`, so
its line endings are never normalized on checkout, keeping that hash stable across platforms.

`fixtures/` holds three golden parity fixtures, one per kind, none of them hand-written and none
...
all three against their schema on every run against the real repo.
...
`tests/GenWave.Host.Tests/Fixtures/golden.theme.json` / `golden.font.json`.
...
so nothing sample-patter- or preview-shaped is required of it. All three share
`author`/`description`/`audience`/`added`, and all three schemas are `additionalProperties: false`.
...
- `<slug>.meta.json` ≤ **64 KB** — same cap for all three kinds
- No size cap is enforced on `<slug>.theme.json` or `<slug>.font.json` text itself — deliberate;
  neither SPEC F103.2 nor F104.2 defines one on the manifest, and the app imposes none on a loaded
  manifest either
...
Each entry also projects `kind` — `"persona"`, `"theme"`, or `"font"` — absent on a persona entry
by design (a missing `kind` means persona, matching every pre-F103.2 entry). A theme or font entry
carries `manifest` (its `<slug>.theme.json` or `<slug>.font.json` path + sha256) where a persona
entry carries `card`; a theme entry additionally carries `preview` (its meta's swatch sets,
projected so the shelf can paint without fetching the manifest); a font entry additionally carries
`assets[]` (its woff2 face(s) + `OFL.txt`, each with `path`/`sha256`/`bytes`) and, optionally,
`family` (copied from the manifest so a zero-fetch shelf listing can show it). Every `assets[]`
`bytes` value is the asset's **real on-disk size**, never a manifest-declared one, and every
`sha256` is likewise recomputed from the file on disk — `tools/build_index.py` never trusts a
manifest's own claims about its own assets.
...
- every JSON Schema above, kind-aware (persona card/meta, theme manifest/meta, font manifest/meta,
  plus `schemas/index.schema.json` for the committed `index.json`)
...
pronunciation-rule sanity (warn-first, red only for the absurd). The diff-scope guard
(`.github/diff_scope_guard.sh`) separately enforces one `entries/<slug>/` + `index.json` per PR.
...
submission-length lint, scoped diff) and the PR template. **Personas and themes are open to
community submission; font packs are Dean-curated only** — see CONTRIBUTING's Font packs section.
...
with a `<slug>.theme.json` manifest in place of the persona card. CI validates the shape; a
